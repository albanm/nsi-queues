...
function AMQPQueuesManager(amqpConnection, callback) {
	var that = this;
	that.amqpConnection = amqpConnection;
	that.correlationCallbacks = {};

	// Prepare the exchange for simple-integration queues
	that.exchange = that.amqpConnection.exchange('nsi.direct', {
		type: 'direct',
		confirm: true
	});
	...
	that.exchange.on('open', function() {
		...
		that.amqpConnection.queue(that.responseQueue, {
			durable: true
		}, function(q) {
...
AMQPQueuesManager.prototype.from = function(queue, readyCallback, callback) {
	...
	that.amqpConnection.queue(queue, {
		durable: true
	}, function(q) {
